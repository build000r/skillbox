...
class RuntimeManagerTests(unittest.TestCase):
    ...
    def _write_fixture(self, repo: Path, include_bundle: bool = True) -> None:
        ...
        (repo / "workspace" / "runtime.yaml").write_text(
            ...
            "        type: mcp_ready\n"
            "        transport: stdio\n"
            "        probe_command: ${SKILLBOX_FWC_BIN} serve-mcp --zone ${SKILLBOX_FWC_ZONE} --connectors ${SKILLBOX_FWC_CONNECTORS}\n"
            "        timeout_seconds: 0.2\n"
            "      log: connectors\n"
            "    - id: dcg-mcp\n"
            "      kind: mcp-bridge\n"
            ...
            "        type: mcp_ready\n"
            "        transport: stdio\n"
            "        probe_command: ${SKILLBOX_DCG_BIN} mcp\n"
            "        timeout_seconds: 0.2\n"
            "      log: connectors\n"
            "  logs:\n"
            "    - id: connectors\n"
    ...
    def test_focus_skill_context_mangling_urls_or_tilde_paths(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            context_text = (client_dir / "context.yaml").read_text(encoding="utf-8")
            self.assertIn("# AUTO-GENERATED by focus. Do not edit.", context_text)
            self.assertNotIn("# Generated:", context_text)
