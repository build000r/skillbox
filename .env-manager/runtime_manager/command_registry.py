...
def default_registry() -> tuple[CommandSpec, ...]:
    ...
    return (
        ...
        CommandSpec(
            ...
            tier=2,
            surface=("cli",),
            summary="Create, list, and verify checksummed tar.gz backups of SKILLBOX_STATE_ROOT.",
            inputs={
                "action": "enum[create|list|verify|drill|restore]?",
                "state_root": "string?",
                "backup_root": "string?",
                "target": "string?",
                "i_understand_data_loss": "boolean?",
                "format": _FORMAT_JSON_TEXT,
            },
            outputs={
                "ok": "boolean",
            ...
            validations=("python3 -m unittest tests.test_state_backup",),
            graph_nodes=("command",),
        ),
        CommandSpec(
            id="runtime.state_backup_drill",
            tier=2,
            surface=("cli",),
            summary="Extract the newest state backup into a temp dir and write restore-drill evidence.",
            inputs={"target": "string?", "state_root": "string?", "backup_root": "string?", "format": _FORMAT_JSON_TEXT},
            outputs={"ok": "boolean", "checks": "object[]", "evidence_path": "string", "manifest": "string"},
            side_effect="local_write",
            risk="low",
            entrypoint="manage.py",
            examples=("python3 .env-manager/manage.py state-backup drill --format json",),
            validations=("python3 -m unittest tests.test_state_backup",),
            graph_nodes=("command",),
        ),
        CommandSpec(
            id="runtime.state_backup_restore",
            tier=2,
            surface=("cli",),
            summary="Restore a verified state backup after pulse, checksum, confirmation, and safety-backup guardrails.",
            inputs={
                "target": "string?",
                "state_root": "string?",
                "backup_root": "string?",
            ...
            outputs={"ok": "boolean", "safety_backup": "object", "checks": "object[]", "state_root": "string"},
        ...
        CommandSpec(
            id="runtime.git_status",
            tier=2,
            surface=("cli",),
            summary="Read-only risk-sorted git status across the repo estate with registry ignore rules, --only filters, and per-row fix commands.",
            inputs={
                "cwd": "string?",
                "format": _FORMAT_JSON_TEXT,
                "only": "string[]?",
        ...
        CommandSpec(
            id="runtime.skill_lint",
            tier=2,
            surface=("cli",),
            summary="Lint repo-local skill override policy for contradictions, floor opt-outs, and stale skill refs.",
            inputs={"client": "string?", "profile": "string[]?", "cwd": "string?", "format": _FORMAT_JSON_TEXT},
            outputs={"ok": "boolean", "findings": "object[]", "summary": "object"},
            side_effect="none",
            risk="low",
            entrypoint="manage.py",
            owner_binary="sbp",
            scopes=("client", "profile", "cwd"),
            examples=("python3 .env-manager/manage.py skill lint --cwd \"$PWD\" --format json",),
            validations=("python3 -m unittest tests.test_override_doctor",),
            graph_nodes=("skill", "repo", "check", "command"),
        ),
        CommandSpec(
            id="runtime.skill_why",
            tier=2,
            surface=("cli",),
            summary="Explain one skill's cwd visibility provenance, including absence and exact durable fix commands.",
            inputs={
                "skill_name": "string",
                "cwd": "string?",
                "no_global": "boolean?",
        ...
        CommandSpec(
            id="runtime.skill_what_if",
            tier=2,
            surface=("cli",),
            summary="Purely simulate effective skill visibility for repo, overlay, pin, opt-out, and machine inputs.",
            inputs={
                "repo": "string",
                "overlay": "string[]?",
                "pin": "string[]?",
            ...
            outputs={
                ...
                "added": "object[]",
                "removed": "object[]",
                "shadowed_by_layer": "object[]",
                "pin_conflicts": "object[]",
            },
            side_effect="none",
            risk="low",
            entrypoint="manage.py",
            owner_binary="sbp",
            scopes=("cwd",),
            examples=("python3 .env-manager/manage.py skill what-if --repo \"$PWD\" --overlay marketing --format json",),
            validations=("python3 -m unittest tests.test_skill_overrides",),
            graph_nodes=("skill", "repo", "overlay", "command"),
        ),
        CommandSpec(
            id="runtime.skill_on",
            tier=2,
            surface=("cli",),
            summary="Durably pin a skill on for the current repo, link it, and return an activation packet.",
            inputs={
                "skill_name": "string",
                "cwd": "string?",
                "dry_run": "boolean?",
            ...
            owner_binary="sbp",
            scopes=("cwd",),
            examples=("python3 .env-manager/manage.py skill on wiki --cwd \"$PWD\" --verify --format json",),
            validations=("python3 -m unittest tests.test_skill_overrides",),
            graph_nodes=("skill", "repo", "command"),
        ),
        CommandSpec(
            ...
            tier=2,
            surface=("cli",),
            summary="Durably pin a skill off for the current repo and unlink current project installs.",
            inputs={
                "skill_name": "string",
                "cwd": "string?",
                "dry_run": "boolean?",
            ...
            owner_binary="sbp",
            scopes=("cwd",),
            examples=("python3 .env-manager/manage.py skill off wiki --cwd \"$PWD\" --dry-run --format json",),
            validations=("python3 -m unittest tests.test_skill_overrides",),
            graph_nodes=("skill", "repo", "command"),
        ),
        CommandSpec(
            id="runtime.skill_heal",
            tier=2,
            surface=("cli",),
            summary="Resolve a real skill source, durably pin it on for the current repo, link it, and return an activation packet.",
            inputs={
                "skill_name": "string",
                "cwd": "string?",
                "dry_run": "boolean?",
            ...
            owner_binary="sbp",
            scopes=("cwd",),
            examples=("python3 .env-manager/manage.py skill heal wiki --cwd \"$PWD\" --format json",),
            validations=("python3 -m unittest tests.test_skill_overrides",),
            graph_nodes=("skill", "repo", "command"),
        ),
        CommandSpec(
            id="runtime.skill_default",
            tier=2,
            surface=("cli",),
            summary="Set repo, cross-repo, or operator-global skill defaults with a dry-run/apply unified diff.",
            inputs={
                "default_action": "enum[on|off]",
                "skill_name": "string",
                "scope": "enum[repo|global|repos|category]",
                "repos": "string?",
                "category": "string?",
                "cwd": "string?",
                "dry_run": "boolean?",
            ...
            owner_binary="sbp",
            scopes=("cwd",),
            examples=("python3 .env-manager/manage.py skill default on wiki --repo --cwd \"$PWD\" --dry-run --format json",),
            validations=("python3 -m unittest tests.test_skill_overrides tests.test_global_contract",),
            graph_nodes=("skill", "repo", "command"),
        ),
        CommandSpec(
            id="runtime.explain",
