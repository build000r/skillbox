...
```json
{
  ...
  "commands": [
    ...
    },
    {
      "json": true,
      "name": "test",
      "notes": "Reads .skillbox/test.yaml. plan/lint are read-only; capsule writes a source capsule to the local store; run/dispatch are DCG-gated and not implemented in this slice.",
      "safe_first_try": "sbp test --format json"
    },
    {
      "json": true,
      "name": "logs",
    ...
    },
    {
      "json": true,
      "name": "family",
      "notes": "Canonical read-only Amp family health contract; dashboard prints the port 6969 URL.",
      "safe_first_try": "sbp family health --json"
    },
    {
      "json": true,
      "name": "registry",
...
| `effective` | CONTRACT | Authoritative visible skill rows for this cwd, including the winning layer and state. |
...
```json
{
  ...
  "fix": {
    ...
    "run_artifact_dir": "<RUNTIME_ROOT>/.skillbox-state/doctor-runs/structure-doctor",
    "supported": true,
